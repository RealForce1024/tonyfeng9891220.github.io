...
## 前言
该文档将演示简单的Go package的开发并介绍[go tool](https://golang.org/cmd/go)(包含fetch,build,install等命令)    
go tool要求以特定的方式组织代码。请详细阅读该文档。你会了解到Go使用安装和最快运行的方法。  
...
注意Go的工作空间组织方式不同于那些每个项目都有单独的工作空间并且每个工作空间与版本控制库紧密相关的其他编程环境。比如Java一个项目一个工作空间也是一个版本库(java工程的最佳实践)。而Go工作空间则是多个版本控制库，每个版本控制库对应一个项目(单个工作空间多个项目代码库)   
...
### GOPATH默认环境变量
**GOPATH环境变量特指工作区的路径。**  
默认情况下在系统家目录(`${HOME}`)下`go`目录。
* Unix `$HOME/go`
* Plan9 `$HOME/go`
* Windows `%USERPROFILE%\go` (通常是`c:\Users\$UserName\go`)
如果你想要在非默认工作区的路径工作，那么你需要设置`GOPATH`环境变量指向那个目录(另一种常见的设置比如是设置`GOPATH=$HOME`)。**注意GOPATH一定不能是Go SDK的安装目录。**
...
#### Windows
工作空间你可以选择自己喜欢的，但是我们将采用`C:work`作为示例演示。注意`GOPATH`不能和你的`GO SDK`安装路径一样
1. 创建`C:\work`目录
...
总结:
记住`GOPATH`并不是Go的安装路径，而是工作空间WorkSpace路径。
`GOPATH`分两种情况
* 默认不设置 `${home}/go`
* 自定义
...
我们将采用`github.com/user`作为`项目根路径`。在工作区存放源码的文件夹下创建该目录：
...
### 第一个go程序
为了编译和运行一个简单的程序，首先我们需要选择`包路径`(我们这里采用`github.com/user/hello`)，在工作区创建相应的目录:
...
注意你可以在系统的任意位置运行上述命令。go工具会根据配置的环境变量`GOPATH`下寻找`github.com/user/hello`包。
...
2. 创建一个方法签名为`func TestXXX(t *testing.T)`的方法  
