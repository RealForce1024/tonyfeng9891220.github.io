...
注意：Go的For循环和其他语言比如C，Java，JavaScript不一样，没有或并不需要括号()括起来其组成的三部分，并且循环体{}不能少。   
...
```
### For是Go语言的"while"
基于此你可以省略分号：Go语言的`for`等同于C语言的`while`
```go
func sum() int{
	sum := 1
	for sum<1000 {
...
### 死循环
如果省略表达式条件将会永远循环下去，所以更简单的表达就是死循环。
...
## if语句
Go的`if`语句块它的`for`循环类似；表达式不需要被括号括起来，但是需要`{}`。  
...
## 带有简短语句块的if
像`for`循环一样，`if`语句在条件表达式之前先执行一个简短的语句。  
...
```
## defer栈
推迟的函数调用被压入栈中。当一个函数返回，它将以last-in-first-out(先进后出或后进先出)的顺序调用。阅读[这篇文章][0]可以学习更多defer相关内容。
...
[0]:https://blog.golang.org/defer-panic-and-recover
