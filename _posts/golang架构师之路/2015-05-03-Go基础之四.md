...
// tpz
```
底层类型为int，被我们绑定了Foo方法，可以看出go的灵巧之处。  
...
//func (tpz *TP) Increase() {
//	*tpz = 100
//}

func (tpz *TP) Increase(num int) {
    // *tpz += num
	*tpz += TP(num)
}
...
## 指针接收器
我们可以使用指针接收器定义方法，从面向对象的角度理解，也可以认为是为接收器定义了方法(更好理解些)。  
这意味着对于某种类型`T`，接收器类型拥有这样的的字面量`*T`。(当然，`T`不能是像`*int`这样的指针)。  
...
func stop(printer Printer) {
	if hp, ok := printer.(HP_Printer); ok { // 接口类型判断
		fmt.Println(hp.name, "stop()")
		return
	}
	fmt.Print("unknow printer")
}
